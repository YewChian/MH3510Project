...
As $Pr(>F) = 0.4213$ which is non-significant, we conclude that model 1 does not lead to a significant improvement over model 2.

```{r}
anova(mlr_model_4, mlr_model_2)
```

Similarly, as $Pr(>F) = 0.3025$ which is non-significant, we conclude that model 4 does not lead to a significant improvement over model 2.
